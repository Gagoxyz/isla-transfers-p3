@php
use Illuminate\Support\Str;
$uuid = Str::random(7);
use App\Models\TransferVehiculo;
use App\Models\TransferViajero;
$emails = TransferViajero::pluck('email');
$vehiculos = TransferVehiculo::all();
@endphp
<!-- Modal -->
<div class="modal fade" id="hotelReturnModal" tabindex="-1" aria-labelledby="customerModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="customerModalLabel">Nueva reserva</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('hotel.reserva.return') }}" method="POST">
                    @csrf
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="rhotelUUID" class="form-label">Localizador</label>
                            <input type="text" value="{{ $uuid }}" name="rhotelUUID" class="form-control" readonly>
                        </div>
                        <div class="col-md-6">
                            <label for="rhotelCustomerEmail" class="form-label">Email del cliente</label>
                            <select class="form-select" name="rhotelCustomerEmail" id="rhotelCustomerEmail">
                                <option selected>Seleccionar...</option>
                                @foreach($emails as $email)
                                <option value="{{ $email }}">{{ $email }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="rhotel_fecha_vuelo_salida" class="form-label">Día de salida</label>
                            <input type="date" name="rhotel_fecha_vuelo_salida" id="rhotel_fecha_vuelo_salida" class="form-control" required>
                        </div>
                        <div class="col-md-6">
                            <label for="rhotel_hora_vuelo_salida" class="form-label">Hora de salida</label>
                            <input type="time" name="rhotel_hora_vuelo_salida" id="rhotel_hora_vuelo_salida" class="form-control" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="rhotel_hora_recogida_salida" class="form-label">Hora de recogida</label>
                            <input type="time" name="rhotel_hora_recogida_salida" id="rhotel_hora_recogida_salida" class="form-control" required>
                        </div>
                        <div class="col-md-6">
                            <label for="rhotel_vehiculo" class="form-label">Vehículo</label>
                            <select class="form-select" name="rhotel_vehiculo" id="rhotel_vehiculo">
                                <option selected>Selecionar...</option>
                                @foreach($vehiculos as $vehiculo)
                                <option value="{{ $vehiculo->id_vehiculo }}">{{ $vehiculo->descripcion }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="rhotel_num_viajeros" class="form-label">Número de pasajeros</label>
                            <input type="number" name="rhotel_num_viajeros" id="rhotel_num_viajeros" class="form-control" required min="1">
                        </div>
                    </div>

                    <div class="d-flex justify-content-end">
                        <button type="button" class="btn btn-secondary me-2" data-bs-dismiss="modal">Cerrar</button>
                        <button type="submit" class="btn btn-primary">
                            <i class="fa-solid fa-download"></i> Registrar reserva
                        </button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>